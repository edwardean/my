...
@property (nonatomic, strong) RatingView *startView;
@end
...
- (void)viewDidLoad
{
    ...
    UIImageView *imgView = [[UIImageView alloc] initWithFrame:CGRectMake(5, 5, 200, 120)];
    self.img = imgView;
    self.img.layer.cornerRadius = 3.0f;
    self.img.layer.masksToBounds = YES;
    [self.scrollView addSubview:_img];
    
    self.startView = [[RatingView alloc] init];
    [self.startView setFrame:CGRectMake(210, 10, 40, 30)];
    [_startView setImagesDeselected:@"0" partlySelected:@"1" fullSelected:@"2" andDelegate:nil];
...
- (void)loadInfo {
    if (_detailDictionary) {
        ...
        float starNum = .1f;
        starNum = [[_detailDictionary objectForKey:@"rate"] floatValue];
        [_startView displayRating:starNum];
    }
}
